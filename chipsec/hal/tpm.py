...
class TPM(hal_base.HALBase):
    ...
    def read_response(self, Locality: int) -> Tuple[tpm_interface.TPM_RESPONSE_HEADER, bytes, bytearray, bytearray]:
    ...
    def _log_register_header(self, register_name: str, locality: str) -> None:
        num_spaces = 32 + (-len(register_name) // 2)  # ceiling division
        self.logger.log('=' * 64)
        self.logger.log("{}{}_{}".format(' ' * num_spaces, register_name, locality))
        self.logger.log('=' * 64)

    def dump_register(self, name: str, locality: str) -> None:
        if name in self.tpm.get_registers():
            self.cs.Cfg.REGISTERS[name]['address'] = str(hex(int(self.cs.Cfg.REGISTERS[name]['address'], 16) ^ LOCALITY[locality]))
