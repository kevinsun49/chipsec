...
from chipsec.hal.tpm_defines import TPM12_defines, TPM20_FIFO_defines
...
class TPM20_FIFO(TPM_BASE):
    ...
    def command(self, commandName: str, locality: str, *command_argv: str) -> None:
        ...
        # Request locality use if needed
        #
        access_address = self.TPM_BASE | Locality | TPM20_FIFO_defines.TPM_ACCESS
        if self.helper.read_mmio_reg(access_address, 4) == TPM20_FIFO_defines.BEENSEIZED:
            self.helper.write_mmio_reg(access_address, 4, TPM20_FIFO_defines.REQUESTUSE)
            requestedUse = True
    ...
    def send_command(self, Locality: int, command: bytes, size: int) -> None:
        ...
        sts_address = self.TPM_BASE | Locality | TPM20_FIFO_defines.TPM_STS
        access_address = self.TPM_BASE | Locality | TPM20_FIFO_defines.TPM_ACCESS

        self.helper.write_mmio_reg(access_address, 1, TPM20_FIFO_defines.REQUESTUSE)
        #
        # Set status to command ready
        #
        sts_value = self.helper.read_mmio_reg(sts_address, 1)
        while (0 == (sts_value & TPM20_FIFO_defines.COMMANDREADY)):
            self.helper.write_mmio_reg(sts_address, 1, TPM20_FIFO_defines.COMMANDREADY)
            sts_value = self.helper.read_mmio_reg(sts_address, 1)
    ...
    def read_response(self, Locality: int) -> Tuple[TPM_RESPONSE_HEADER, bytes, bytearray, bytearray]:
        ...
        while data_avail == '0':
            sts_value = self.helper.read_mmio_reg(sts_address, 1)
            self.helper.write_mmio_reg(sts_address, 1, TPM20_FIFO_defines.DATAAVAIL)
            data_avail = bin(sts_value & (1<<4))[2]

        while count < TPM20_FIFO_defines.HEADERSIZE:
            sts_value = self.helper.read_mmio_reg(sts_address, 4)
            burst_count = ((sts_value>>8) & 0xFFFFFF)
            burst_index = 0
            while (burst_index < burst_count) and (count < TPM20_FIFO_defines.HEADERSIZE):
                header_blob.append(self.helper.read_mmio_reg(datafifo_address, 1))
                count += 1
                burst_index += 0x1

        header = TPM_RESPONSE_HEADER(*struct.unpack_from(TPM20_FIFO_defines.HEADERFORMAT, header_blob))

        count = 0
        if header.DataSize > 10 and header.ReturnCode == 0:
            length = header.DataSize - TPM20_FIFO_defines.HEADERSIZE
            while count < length:
                sts_value = self.helper.read_mmio_reg(sts_address, 4)
                burst_count = ((sts_value>>8) & 0xFFFFFF)
                burst_index = 0
